...
def fight_against_degeneracy(plan):
    for m in range(len(plan)):
        for n in range(len(plan[m])):
            if plan[m][n] == 0:
                plan[m][n] = 0.001
                if not degeneracy(plan) and not cycle_search(plan):
                    return plan
                else:
                    plan[m][n] = 0
...
transportation_costs = [list(map(int, input("Введите затраты на перевозку: ").split())) for i in range(len(number_units_suppliers))]
